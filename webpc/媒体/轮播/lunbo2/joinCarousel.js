...
;(function($){
    var Caroursel = function (caroursel){
        ...
        this.posli=this.posterList.find("li")
        ...
        if(this.setting.isAutoplay){
            // this.autoPlay();
            this.caroursel.hover(function(){clearInterval(self.timer)},function(){})
        }
    ...
    Caroursel.prototype = {
        ...
        rotateAnimate:function(type){
            ...
            if(type == "left"){//向左移动
                ...
                this.posterItems.each(function(i){
                    ...
                    $(this).removeClass("a1 a2 a3 a4 a5");
                    $(this).addClass('a'+zIndexArr[i])
                });
            }
            if(type == "right"){//向右移动
        ...
        setFirstPosition:function(){
            ...
            var zindex=100;
            //设置两个按钮的样式
            // this.prevBtn.css({"width":width , "height":this.setting.height,"zIndex":zindex});
            // this.nextBtn.css({"width":width , "height":this.setting.height,"zIndex":zindex});
...
//              leftHeight = posterHeight*_self.setting.scale;
  				leftWidth =parseInt(posterWidth * (2.2/(i+2)));
                leftHeight = parseInt(posterHeight*(2.2/(i+2)));
//              var rightWidth = parseInt(posterWidth * (2.5/(j+1)));
//              var rightHeight = parseInt(posterHeight*(2.5/(j+1)));
...
//              var rightHeight = posterHeight*_self.setting.scale;
    		var rightWidth = parseInt(posterWidth * (3/(j+2)));
            var rightHeight = parseInt(posterHeight*(3/(j+2)));
