function page_ctrl(data_obj) {
  var obj_box=(data_obj.obj_box!== undefined)?data_obj.obj_box:function () {
	...
	var obj_list=(data_obj.obj_list!== undefined)?data_obj.obj_list:function () {
		return;
	};//翻页容器dom对象,必要参数
  ...
  function page_even() {
    ...
    function change_content() {
			var startRow = (current_page - 1) * per_num + 1;   //currentPage 当前页数   pageSize  每页显示条数
			var endRow = current_page * per_num;
			endRow = (endRow > total_item) ? total_item : endRow;  //num 总条数
			var tablepag = new Array();
			for (let index = startRow; index <= endRow; index++) {
					tablepag.push(data_obj.table[index - 1])
			}
			tabletr(tablepag);
    }
		...
		function tabletr(arr) {
			if(arr!=''){
				data_obj.success(arr)
			}else{
				$(obj_list).html('<p class="NoData">暂无数据</p>')
			}
		}
    ...
    if(current_page==total_page){
      append_html+='<button class="page_num current_page">'+(i+1)+'</button>';
    }else if(current_page!=total_page&&total_page!=0){
      append_html+='<button class="page_num">'+(i+1)+'</button>';
    }else if(total_page==0){
			append_html+='<button class="page_num">'+''+'</button>';
		}
    append_html+='<button class="next_page">></button><span class="page_total">';
