<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>编辑服务</el-breadcrumb-item>
    </el-breadcrumb>
...
<script>
  export default {
    ...
    created() {
      this.getCraftsList();
      this.getServiceBaseInfo({id:Number(this.$route.query.id)});
    ...
    methods: {
      ...
      getServiceBaseInfo(params){
        this.$http.post('/operation/service/getServiceInfo', params).then((res) => {
          if(res.data.code == 200) {
      ...
      pageFormInit() {
        this.pageOne.serviceTypes = this.serviceInfo.serviceCatalogList;

        this.pageOne.baseInfoForm.name = this.serviceInfo.serviceInfo.serviceName;
        this.pageOne.baseInfoForm.type = Number(this.serviceInfo.serviceInfo.serviceCatalog.id);
        this.pageOne.baseInfoForm.minTime = this.serviceInfo.serviceInfo.periodMin;
        this.pageOne.baseInfoForm.maxTime = this.serviceInfo.serviceInfo.periodMax;
        this.pageOne.baseInfoForm.periodUnit = this.serviceInfo.serviceInfo.periodUnit;
        this.pageOne.baseInfoForm.invoiceTemplateId = this.serviceInfo.serviceInfo.invoiceTemplateId;

        this.serviceInfo.serviceProdecureList.map((item) => {
          var dataItem = {name:item.stepName,crafts:'',craftsIds:[],configured:false,id:item.id};

          var craftsNameArr = [];
          item.techniqueList.map((el) => {
            dataItem.craftsIds.push(el.id);
            craftsNameArr.push(el.techniqueName);
          })
          dataItem.crafts = craftsNameArr.toString();

          this.pageTwo.tableData.push(dataItem);
        })
   ...
   //     this.getCraftsList();
      ...
      addRow(index){
        this.pageTwo.tableData.splice(index+1,0,{name:'',crafts:'',craftsIds:[],configured:false, id:null});
      },
      moveUpRow(index){
        if(index != 0){
      ...
      addService(){
        ...
        this.$http.post('/operation/service/updateServiceAndProcedures', service).then((res) => {
          if (res.data.code == 200) {
            this.$message({
              type: 'success',
