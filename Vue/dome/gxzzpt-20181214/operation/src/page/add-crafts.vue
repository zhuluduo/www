<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>添加工艺</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="step-page" v-show="pageOne.visible">
      <div class="step-box-1">
        ...
        <el-form ref="craftsBaseInfoForm" :model="pageOne.craftsBaseInfoForm" :rules="pageOne.rules" label-width="80px">
          ...
          <el-form-item label="工艺图" class="upload-label" prop="pictureName">
      ...
      <div class="step-box-2">
        ...
        <div class="select">
          <div class="select-item">
            ...
            <div class="material-box" v-for="item in pageTwo.rawMaterialCheckList">
              <div class="material-type"  v-show="item.materials.length">{{item.catalogName}}</div>
              <div class="material-item">
                <el-tag closable v-for="child in item.materials" @close="tagCloseHandler(child, item)">{{child.materialName}}</el-tag>
              </div>
            </div>
          </div>
          <div class="select-item">
        ...
        <div class="step-btn">
          <el-button type="primary" @click="pageTwo.visible=false;pageOne.visible=true">上一步</el-button>
      ...
      <el-dialog class="add-material-dlg"
        ...
        <div>
          <el-tabs type="card" v-model="pageTwo.rawDlgTab">
            <el-tab-pane :label="rawMaterial.catalogName" :name="String(index)" v-for="(rawMaterial,index) in pageTwo.rawMaterialList">
      ...
      <el-dialog class="add-material-dlg"
        ...
        <div>
          <el-tabs type="card" v-model="pageTwo.conDlgTab">
            <el-tab-pane :label="cMaterial.catalogName" :name="String(index)" v-for="(cMaterial,index) in pageTwo.consumableMaterialList">
              <div class="materials-wrapper">
                <div class="materials">
    ...
    <div class="step-page" v-show="pageThr.visible">
      <div class="step-box-3">
        ...
        <el-table
          ...
          <el-table-column
            ...
            label="参数值">
            <template slot-scope="scope">
              <span>{{scope.row.cfgValueStr}}</span>
      ...
      <el-dialog
        title="参数值"
        :visible.sync="pageThr.dlgVisible"
        width="500px">
        <div>
          <el-form labelPosition="top" ref="form" label-width="80px">
            <el-form-item v-for="(item,index) in pageThr.dlgForm.inputList">
              <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getMaterialType1() {
      this.$http
        .post("/operation/material/getAllByMaterialType", { type: 103010 })
        .then(res => {
          if (res.data.code == 200) {
    ...
    getMaterialType2() {
      this.$http
        .post("/operation/material/getAllByMaterialType", { type: 103020 })
        .then(res => {
    ...
    configParams() {
      ...
      ].cfgValueStr = cfgValues.toString();
      this.pageThr.tableData[this.pageThr.currRow].cfgValues = cfgValues;
      this.pageThr.tableData[
        this.pageThr.currRow
    ...
    openCfgDlg(row, scope) {
      ...
      if (this.pageThr.tableData[scope.$index].cfgValues.length > 0) {
        this.pageThr.dlgForm.inputList = [];
        this.pageThr.tableData[scope.$index].cfgValues.map(item => {
    ...
    addCrafts() {
      ...
      this.pageThr.tableData.map(item => {
        if (item.name && item.configured) {
          var craftParams = {
            defaultValue: item.defValue,
            techniqueParamName: item.name,
            techniqueParamPicture: "",
            techniqueParamInfo: "",
            valueList: item.cfgValues
          };
          craft.params.push(craftParams);
        }
      });
      this.$http.post("/operation/technique/add", craft).then(res => {
        if (res.data.code == 200) {
          this.$message({
            type: "success",
            message: "添加成功"
