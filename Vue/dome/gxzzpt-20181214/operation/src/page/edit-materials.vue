<template>
  <div>
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item :to="{ path: '/main/materials-manage'}">物料管理</el-breadcrumb-item>
        <el-breadcrumb-item>编辑物料</el-breadcrumb-item>
      </el-breadcrumb>
      <div class="step-box">
          <div class="step step1" v-show="currentStep==0?true:false">
              <p class="step-header">第一步、填写物料基本信息</p>
              <div>
                    <el-form label-position="top" label-width="80px" :model="form1" :rules="formRules1" ref="form1">
                        <el-form-item label="物料名称" class="required" prop="materialName">
                            <el-input v-model="form1.materialName"></el-input>
                        </el-form-item>
                        <el-form-item label="物料类别" class="required" prop="catalogIds">
                        ...
                        <el-form-item label="物料类型" class="required">
          ...
          <div class="step step2" v-show="currentStep==1?true:false">
            <p class="step-header">第二步、配置物料默认参数</p>
            <div>
                <el-form label-position="top" label-width="80px" :model="form2" ref="form2">
                    <el-form-item :label="ele.paramName+'（'+ele.valueUnit+'）'" class="required" v-for="(ele,i) in form2.internalParams" :key="i" :prop="'internalParams.' + i + '.value'" :rules="check">
                     ...
                     <!-- <el-form-item label="物料单价（元/克）" class="required">
                        <el-input v-model="form1.name"></el-input>
                    </el-form-item>
                     <el-form-item label="加工单价（元/cm³）" class="required">
                        <el-input v-model="form1.name"></el-input>
                    </el-form-item>
                     <el-form-item label="物料壁厚（cm）" class="required">
                        <el-input v-model="form1.name"></el-input>
                    </el-form-item> -->
                </el-form>
            </div>
          ...
          <div class="step step3" v-show="currentStep==2?true:false">
            <p class="step-header">第三步、配置物料自定义参数</p>
                ...
                <el-table :data="tableData" style="width: 100%" @header-click="addPara">
                    ...
                    <el-table-column prop="value" label="值">
                        <template slot-scope="scope">
                            ...
                            <span>{{scope.row.valueList?scope.row.valueList.join(','):""}}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="" label="+" type="add" :label-class-name="cln">
                        <template slot-scope="scope">
                            <span class="table-operator" @click="removeRow(scope.row,tableData)">删除</span>
                        </template>
                    </el-table-column>
               </el-table>
            </div>
      ...
      <el-dialog title="值" :visible.sync="showEditForm" labelPosition="left" width="500px">
...
export default {
  ...
  methods: {
    ...
    getDetail(id) {
      ...
      this.$http
        .post("/operation/material/get", data)
        .then(res => {
          if (res.data.code == 200) {
            ...
            this.form1 = {
              ...
              detailUrl: this.detail.detailUrl || "",
              picture: this.detail.picture,
              type: this.detail.type,
              catalogIds: this.detail.catalog[0].catalogId
            };
            this.form2.internalParams.map((ele, index) => {
              this.detail.internalParams.map((el, i) => {
                if (el.paramCode == ele.id) {
                  ele.value = el.value;
                  this.$set(ele,"paramId",el.id);
                }
              });
            });
            this.tableData = this.detail.customParams||[];
          } else {
            this.$message({
              type: "error",
    ...
    submitForm() {
      ...
      this.$http
        .post("/operation/material/edit", ajaxData)
        .then(res => {
          if (res.data.code == 200) {
    ...
    removeByAjax(item,list){
      var data = {
        id:item.id
      }
      this.$http.post('/operation/material/deleteMaterialParam',data).then(res=>{
        if(res.data.code == 200){
