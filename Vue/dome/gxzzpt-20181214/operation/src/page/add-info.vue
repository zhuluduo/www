  ...
  <div>
      ...
      <el-dialog 
        ...
        <div>
          ...
          <div class="form-row">
            <div class="left">
              <el-form label-position="left" ref="catalogForm" :rules="catalogForm.rules" :model="catalogForm">
            ...
            <div class="right">
              <el-button type="primary" @click="addSort('catalogForm')">添加</el-button>
            </div>
          </div>
        </div>
        <ul class="form-list">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      catalogForm: {
        ...
        list: [],
        rules:{
          catalogName:[ { required: true, message: "请填写分类名称", trigger: "blur" },]
        },
      },
      techniqueData:[],
      form: {
  ...
  methods: {
    ...
    submit() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.$http
            .post("/operation/information/add", data)
            .then(res => {
              if (res.data.code == 200) {
    ...
    upload(img) {
      this.form.coverPicturl = img.imgUrl;
    },
    changeModule(i) {
    ...
    getAllCatalog() {
      this.$http.post("/operation/module/all").then(res => {
        if (res.data.code == 200) {
          ...
          this.catalogList = this.moduleList[0].catalogList;
          this.form.moduleId = this.moduleList[0].id;
          this.form.catalogId = this.catalogList[0].id;
          this.catalogList.map(item => {
            this.$set(item, "edit", false);
          });
