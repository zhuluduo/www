<template>
    <div class="distributing-requirement">
        ...
        <div class="table">
              <el-table :data="tableData" border style="width: 100%" header-row-class-name="co-f1" v-loading="loading" element-loading-text="数据加载中">
                ...
                <el-table-column prop="" label="操作" align="center" width="142px">
                    <template slot-scope="scope">
                        <span class="table-btn modal-name" @click="$router.push({path:'/main/requirement-details',query:{'id':scope.row.id, from:'/main/distributing-requirement'}})">需求详情</span>
                        <span class="table-btn modal-name" @click="$router.push({path:'/main/dispatch-order',query:{'id':scope.row.id}})">分派</span>
                    </template>
                </el-table-column>
            </el-table>
        </div>
...
export default {
    ...
    methods:{
        ...
        getToBeAllocatedList(){
            this.loading=true;
            this.$http.post("/operation/requirement/getToBeAllocatedList",this.ajaxData).then(res => {
