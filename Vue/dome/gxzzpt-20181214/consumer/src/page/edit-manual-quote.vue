                ...
                <div class="form-content-item" v-for="(partForm, index) in partForms" :ref="'partFormItem' + index" :key="'part-form-' + index">
                    ...
                    <div class="form-box">
                        <el-form :rules="partRules" :model="partForm" :ref="'partForm' + index">
                            ...
                            <el-form-item>
                                <el-col :span="11">
                                    <el-form-item label="目标单价：" label-position="left" label-width="95px" prop="expectPrice">
...
export default {
    ...
    created() {
        ...
        this.techniqueType = this.$route.query.techniqueType;
        this.baseForm.data.techniqueId = Number(this.$route.query.techniqueId);
        this.requirementId = Number(this.$route.query.requirementId);
        this.baseForm.data.techniqueNames = this.$route.query.techniqueName;
        this.treeModal.btnName = this.baseForm.data.techniqueNames;
        this.treeModal.SelectDatas.push(this.baseForm.data.techniqueId);
        this.getData();
    },
    methods:{
        ...
        getData() {
            this.$http.post('/custom/requirement/get', {id: this.requirementId}).then(( res ) => {
                if ( res.data.code == 200 ) {
                    ...
                    this.baseForm.data.industryId = this.requirmentData.industryInfo?this.requirmentData.industryInfo.id:'';
                    this.baseForm.data.industryName = this.requirmentData.industryInfo?this.requirmentData.industryInfo.industryName:'';
                    ...
                    this.requirmentData.requirementItemList.map(( item ) => {
                        var partItem = {
                            ...
                            showExpectPrice:item.showExpectPrice||true,
                            modelFileIds:[],
                            planeGraphFileId:item.planeGraphFileInfo?item.planeGraphFileInfo.id:'',
                            otherFileIds:[],
                            description:item.description||'',
                            isLadderPrice:item.isLadderPrice,
                            num1: item.isLadderPrice?item.ladderPriceInfo[0].from:null,
                            num2: item.isLadderPrice?item.ladderPriceInfo[0].to:null,
                            num3: item.isLadderPrice?item.ladderPriceInfo[1].to:null,
                            planeGraphFileName:item.planeGraphFileInfo?item.planeGraphFileInfo.fileName:'',
                            modelFileNames:'',
                            modelFiles:[],
                            otherFiles:item.otherFileList||[],
                        };

                        console.log('item.material',item);

                        if ( item.modelFileList ) {
                            item.modelFileList.map((el) => {
                                var file = {
                                    fileId: el.id,
                                    orignFileName: el.fileName,
                                    thumbnailUrl:'',
                                }
                                partItem.modelFiles.push(file);
                                partItem.modelFileIds.push(el.id);
                            })
                        }

                        if ( item.otherFileList ) {
                            item.otherFileList.map((el) => {
                                partItem.otherFileIds.push(el.id);
                            })
                        }

                        this.partForms.push( partItem );
        ...
        //获取供应商列表
        getCompanys( keyword ) {
            this.$http.post('/getManufacturerList',{keyword:keyword, techniqueTypeList:[this.baseForm.data.techniqueId]}).then(( res ) => {
                if ( res.data.code ==200 ) {
       ...
       //上传模型文件前
        beforeUploadModelFile( file ) {
            ...
            if ( this.baseForm.data.techniqueType == 106010 ) {
