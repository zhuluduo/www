<template>
    <div :class="SwitchStates==true||IndustryType==true?'Tree-common':'Trees-common'">
      <el-button type="primary" @click="BtnDialog" size="small" class="btn">{{btnNames}}</el-button>
      <slot name="cont"></slot>
      <el-dialog
        :title="title"
        class="dialog-box"
        ...
        <div class="dialog-cont" v-loading="loading" element-loading-text="数据加载中">
          <div class="dialog-tag">
            ...
            <div class="dialog-tag-list"> 
              <el-tag size="medium" closable v-for="item in checkData" :key="item.id" @close="handleClose(item)">{{item.techniqueName}}</el-tag>
            </div>
          </div>
          <p class="xian"></p>
      ...
      getTechnique(){
        ...
        for (var i = 0; i < len; i++) {
          ...
          if(newJson[i].children==undefined&&newJson[i].techniqueList==undefined&&newJson[i].disabled==undefined){
            ...
            // var response = window.JSON.stringify(this.dataInfo)
