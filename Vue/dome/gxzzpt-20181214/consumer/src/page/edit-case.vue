...
export default {
  components: {
    ...
    data() {
        return {
            ...
            rules:{
                ...
                techniqueId: [{required: true,message:'请选择主工艺',trigger:'change'}],
    ...
    methods: {
        ...
        getData() {
            this.$http.post('/custom/manufacturer/productCase/get', {id: this.id}).then(( res ) => {
                if ( res.data.code == 200 ) {
                    ...
                    this.formData.industryId = data.industryInfo.id;
                    this.formData.technique = data.technique;
                    this.formData.material = data.material || '';
                    this.industrys = data.industryList || [];
                    this.formData.priceScope = data.priceScope;
                    this.formData.productionCapacity = data.productionCapacity;
                    this.formData.pictureUrls = data.pictureUrls || [];
                    this.formData.description = data.description || '';
                    this.Tree.SelectDatas=[res.data.data.techniqueInfo.id];
                    this.formData.techniqueId=res.data.data.techniqueInfo.id;
                    this.Tree.btnName=res.data.data.techniqueInfo.techniqueName
                } else {
        ...
        uploadImg( e ) {
            if ( this.formData.pictureUrls.length >= 4 ) {
                this.$error('只能上传4张');
            } else {
        ...
        submit() {
            this.$refs.caseForm.validate(( valid ) => {
                if ( valid ) {
                    ...
                    this.$http.post('/custom/manufacturer/productCase/update', this.formData).then(( res ) => {
                        if ( res.data.code == 200 ) {
