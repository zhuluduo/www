    ...
    <div id="artificial-quotation">
        <div class="content">
            <div class="crumb-bread">
                ...
                <span class="right-arrow"><i class="el-icon-arrow-right"></i></span> 
                <span class="router-link">人工报价</span>
                <span class="right-arrow"><i class="el-icon-arrow-right"></i></span>
            ...
            <div class="form-wrapper">
                <div class="form-content-item" ref="baseFormItem">
                    ...
                    <div class="form-box">
                        <el-form :rules="baseForm.rules" :model="baseForm.data" ref="baseForm" label-position="left">
                            ...
                            <el-form-item label-position="top">
                                <el-col :span="11">
                                    <el-form-item label="主工艺：" label-position="left" label-width="95px" prop="requirementType" required>
                                        <el-select :disabled="baseForm.data.requirementType==106010" @change="requirementChangeHandler" v-model="baseForm.data.requirementType">
                ...
                <div class="step-item">
                    ...
                    <a href="" style="text-decoration:underline;" class="link-with-underline">点击我下载询盘导入模板</a>
                </div>
...
export default {
        ...
        return {
            ...
            baseForm:{
               ...
               rules:{
                    ...
                    requirementType: [{required: true,message:'请选择主工艺',trigger:'change'}],
    ...
    created() {
        ...
        if ( this.$route.query.requirementId ) {
            this.requirementId = Number(this.$route.query.requirementId);
            this.getData();
        } else {
    ...
    methods:{
        ...
        getData() {
            this.$http.post('/custom/requirement/get', {id: this.requirementId}).then(( res ) => {
                if ( res.data.code == 200 ) {
                    this.requirmentData = res.data.data;
                    this.baseForm.data.industryId = this.requirmentData.industryInfo.id;
                    this.baseForm.data.offerDeadlineTime = this.requirmentData.offerDeadlineTime||'';
                    this.baseForm.data.contactName = this.requirmentData.contactName||'';
                    this.baseForm.data.contactEmail = this.requirmentData.contactEmail||'';
                    this.baseForm.data.contactPhone = this.requirmentData.contactPhone||'';
                    this.baseForm.data.enquiryType = this.requirmentData.enquiryType||'';
                    this.baseForm.data.settlementType = this.requirmentData.settlementType||'';
                    this.baseForm.data.settlementPeriod = this.requirmentData.settlementPeriod||'';
                    this.baseForm.data.description = this.requirmentData.description||'';
                    this.baseForm.data.companys = this.requirmentData.enquiryType==230020?this.requirmentData.tempDispatchCompanyIds:[];
                    this.baseForm.data.expressMode = this.requirmentData.expressMode;

                    this.enquiryTypes.map(( item ) => {
                        if ( item.id == this.baseForm.data.enquiryType ) {
                            item.isCheck = true;
                        } else {
                            item.isCheck = false;
                        }
                    ...
                    this.requirmentData.requirementItemList.map(( item ) => {
                        var tableItem = { itemNumber: item.itemNo, itemName:item.itemName, estimateCount: item.estimateCount, unit:item.unit, 
                            picNumber: item.photoNo, versionNumber:item.version,fileList:item.otherFileList};
        ...
        initRequirementType() {
             if ( this.$route.query.requirementType ) {
                this.baseForm.data.requirementType = Number(this.$route.query.requirementType);
                if ( this.baseForm.data.requirementType != 106010 ) {
                    this.requireTypes.map(( item, index ) => {
                        if ( item.id == 106010 ) {
                            this.requireTypes.splice(index, 1);
                        }
                    })
                }
            } else {//
                ...
                if ( this.requireTypes.length > 0 ) {
                    this.baseForm.data.requirementType = this.requireTypes[0].id;
                    this.baseForm.data.requirementName = this.requireTypes[0].name;
                }
            }
            if ( this.$route.query.requirementName ) {
                this.baseForm.data.requirementName = this.$route.query.requirementName;
            }
            if ( this.$route.query.industryName ) {
                this.baseForm.data.industryName = this.$route.query.industryName;
            }
        ...
        requirementChangeHandler(value) {
            this.requireTypes.map(( item ) => {
                if ( value == item.id ) {
                    this.baseForm.data.requirementName = item.name;
                }
            })
            this.getCompanys( '' );
            this.baseForm.data.materialId = null;
            this.getMaterials();
        },
        ...
        //获取供应商列表
        getCompanys( keyword ) {
            this.$http.post('/getManufacturerList',{keyword:keyword, techniqueTypeList:[this.baseForm.data.requirementType]}).then(( res ) => {
                if ( res.data.code ==200 ) {
        ...
        //获取材质列表
        getMaterials() {
            this.$http.post('/custom/Material/getManualQuotationMaterial',{materialTechniqueType: this.baseForm.data.requirementType}).then(( res ) => {
            if ( res.data.code == 200 ) {
        ...
        itemParamsHandler( isDraft ) {
            var params = {
                ...
                requirementType: this.baseForm.data.requirementType,
                requirementName: this.baseForm.data.requirementName,
