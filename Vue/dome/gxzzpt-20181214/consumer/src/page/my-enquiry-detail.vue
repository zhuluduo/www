    ...
    <div id="require-detail">
        <div class="content">
            <div class="crumb-bread">
                ...
                <span class="router-link" @click="$router.push({path:'/index'})">我的询盘</span>
                <span class="right-arrow"><i class="el-icon-arrow-right"></i></span>
                <span class="curr">询盘详情</span>
            </div>
            <div class="require-number clearfix">
                <div class="number">需求编号：{{data.requirementNo}}</div>
                <div class="state">{{data.requirementStatusText}}</div>
            </div>
            <div class="content-item">
                ...
                <div class="content-item-box">
                    <div class="row-info">
                        ...
                        <div class="info-item ellipsis" v-if="data.deliveryProvince">送货地区：{{data.deliveryProvince + data.deliveryCity}}</div>
                    </div>
                    <div class="row-info">
                            ...
                            <tr class="tr2">
                                <td colspan="7">
                                    <div class="td-wrapper">
                                        <div class="file-row" v-if="partItem.modelFileList||partItem.planeGraphFileInfo||partItem.otherFileList">
                                            <div class="file-type ellipsis" v-if="partItem.modelFileList">3D模型：<a target="_blank" :href="model.fileUrl" v-for="model in partItem.modelFileList" :key="model.fileUrl">{{model.fileName}}</a></div>
                                        ...
                                        <div class="multi-row-info-item">
                                            <label style="width:50px;" for="">说明：</label>
                                        ...
                                        <div class="analysic-box" v-if="data.enquiryType==230010">
                                            ...
                                            <div class="content-box">
                                                ...
                                                <div class="multi-row clearfix" style="display:flex;">
                                                    <div>说明：</div>
                                                    <div >{{partItem.analysisRemark?partItem.analysisRemark:'无'}}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
export default {
    ...
    methods: {
        getData() {
            this.$http.post('/custom/requirement/get', {id: this.requireId}).then(( res ) => {
