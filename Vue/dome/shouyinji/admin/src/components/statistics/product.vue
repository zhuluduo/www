<template>
  <div class="content-list">
    ...
    <el-row :gutter="2">
      ...
      <el-col :span="2">
        <el-select v-model="params.storeId"   clearable placeholder="店铺名称" size="small">
      ...
      <el-col :span="3" class="tool-bar" align="right">
      ...
      <el-col :span="24">
        <el-table v-loading="loading" stripe :data="productList" border @sort-change="sortList" :default-sort = "{prop: 'quantity', order: 'descending'}" show-summary :summary-method="productSummary">
          <el-table-column label="单品排行" align="center">
            ...
            <el-table-column sortable="custom" prop="firstCategoryName" label="一级分类" width="120"/>
            <el-table-column sortable="custom" prop="secondCategoryName" label="二级分类" width="120"/>
            <el-table-column sortable="custom" prop="price" label="商品售价" width="120"/>
            ...
            <el-table-column sortable="custom" prop="profit" label="毛利" width="90"/>
          </el-table-column>
        </el-table>
        <el-pagination
  ...
  export default{
    ...
    methods:{
      ...
      loadParentCategory(parentId) {
        let url = bus.host+'/admin/api/category/list/'+parentId;
      ...
      loadProduct(){
        ...
        let url=bus.host+'/admin/api/statistics/product?page='+(this.productPage.currentPage-1);
      ...
      loadCategory(){
        ...
        this.params.ymdBegin=this.ymdBegin;
        this.params.ymdEnd=this.ymdEnd;
        let url=bus.host+'/admin/api/statistics/category?page='+(this.categoryPage.currentPage-1);
        if(this.listSort.order!=null){
          url+='&sort='+this.listSort.prop+','+this.listSort.order;
        }
        this.$axios.post(url,this.params).then((res)=>{
      ...
      // 导出销售记录
      exportStatistics(){
        let url=bus.host+'/admin/api/statistics/export';
        if(this.listSort.order!=null){
          url+='?sort='+this.listSort.prop+','+this.listSort.order;
        }
        this.loading = true;
