  ...
  <div>
    ...
    <el-row :gutter="2" style="margin-top:10px;">
      ...
      <el-col :sm="8" :md="6" class="tool-bar" align="right" style="float:right;">
        ...
        <el-button type="primary" @click="save(list,false)" size="small" icon="check">保存</el-button>
        <el-button v-if="detail.status===0" type="danger" size="small"
                   @click="confirm(list,true)" icon="check">确认入库</el-button>
      </el-col>
    </el-row>
    <el-row>
      <el-col>
      ...
      validate(rows) {
        ...
        try{
          rows.forEach((e)=>{
            ...
            if(e.productionDate==''||e.productionDate==null||e.productionDate==undefined){
            ...
            if(!/^\d+(\.\d+)?$/.test(e.purchasePrice)||e.purchasePrice==0){
