  ...
  <div>
    <el-row class="breadcrumb-border">
      <el-col>
        <el-breadcrumb separator=">">
          ...
          <el-breadcrumb-item :to="{ path: 'list' }">库存盘点</el-breadcrumb-item>
    ...
    <el-row class="data-table data-table-pd" :gutter="2">
      ...
      <el-col :span="10" align="right">
        ...
        <el-button v-if="this.$route.query.Status==1 || this.$route.query.Inventory==1" type="primary"
                   @click="export_check" size="small" icon="caret-right" :loading="loading">导出盘点
        </el-button>
        <el-button v-if="this.$route.query.Status==0 || this.$route.query.Inventory==0" type="primary"
                   @click="End_check" size="small" icon="check" :loading="loading">结束盘点
        </el-button>
      </el-col>
    </el-row>
    <el-row :gutter="2">
      <el-col :span="24" v-loading="loading">
        <el-table stripe border :data="list" v-loading="loading" element-loading-text="数据加载中" @sort-change="sortList"
          ...
          <el-table-column prop="name" label="商品名称" width="300" sortable="custom"></el-table-column>
          <el-table-column prop="barcode" label="商品条码" sortable="custom"></el-table-column>
          <el-table-column prop="spec" label="规格" sortable="custom"></el-table-column>
          <el-table-column prop="pkg" label="单位" sortable="custom"></el-table-column>
          <el-table-column prop="secondCategory.name" label="分类"></el-table-column>
          <el-table-column prop="systemInventory" label="系统库存"></el-table-column>
          <el-table-column label="盘点库存">
            <template scope="scope">
              <el-tooltip :disabled="disabled"  :content="content" placement="right" effect="light">
                <el-input v-model="scope.row.stocktaking"  type="number"  @focus="closeTooltip()" @blur="submitForm(scope.row,scope.row.id,scope.row.productId, scope.row.systemInventory,scope.row.stocktaking)"  @change="changeHint(scope.row)" size="mini"/>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column prop="quantity" label="缺失数量"></el-table-column>
          <!--<el-table-column prop="quantity" label="缺失数量">-->
            <!--<template scope="scope">-->
              <!--<el-input  v-model="scope.row.quantity"  disabled="disabled" :readonly="true" size="mini"/>-->
            <!--</template>-->
          <!--</el-table-column>-->
          ...
          <el-table-column prop="checkStatus" label="状态">
            <template scope="scope">
              <el-tag v-if="scope.row.checkStatus==1" type="success">已盘点</el-tag>
              <el-tag v-if="scope.row.checkStatus==0" type="warning">未盘点</el-tag>
            </template>
          </el-table-column>
         <!-- <el-table-column v-if="this.$route.query.Status==0 || this.$route.query.Inventory==0" label="操作" width="100">
            <template scope="scope">
              <el-button  :plain="true" type="warning"
                         @click="submitForm(scope.row,scope.row.id,scope.row.productId, scope.row.systemInventory,scope.row.stocktaking)"
                         size="small" :disabled="scope.row.id=='DEFAULTID'?true:false" icon="edit">库存修正
              </el-button>
            </template>
          </el-table-column>-->
        </el-table>
            ...
            <el-row :gutter="20">
              <el-form :model="ruleForm" class="ruleForm_cent">
                ...
                <el-form-item scope="scope" label="盘点库存：">
                  <span > <el-input  @keyup.enter.native="submitForm" v-model.trim="ruleForm.stocktaking" auto-complete="off" class="form-inline"
...
<script>
  ...
  export default {
    ...
    methods: {
      // 修正库存
      /*  modifyInventory(row,baseId,productId,systemInventory,stocktaking){
          console.log('baseId='+baseId);
          console.log('productId='+productId);
          console.log('systemInventory='+systemInventory);
          console.log('stocktaking='+stocktaking);
          row.quantity=stocktaking-systemInventory;
        },*/
      /*分页*/
      ...
      changeHint(row){
        let stocktaking = row.stocktaking;
        if (stocktaking === null || stocktaking === '' || stocktaking === undefined) {
          this.disabled=false;
          row.stocktaking=null;
          return false;
        }
        console.log(row.stocktaking)
        if (stocktaking != null) {
          if (!/^\d+$/.test(stocktaking)) {
            console.log(row.stocktaking)
            this.disabled=false;
            row.stocktaking=null;
            return false;
      ...
      listTable() {
        if (this.$route.query.Id) {
          ...
          this.listQuery.productStatus = 1;
          ...
          this.$http.post(this.url + pg, this.listQuery, {}).then((res) => {
            ...
            ListTab.forEach(base => {
              let products = base.products;
              base.systemInventory = 0;
              if (products != null) {
                base.systemInventory = products[0].inventory;
                base.productId = products[0].id;
              }
            });
            this.list = ListTab;
            console.log(this.list[0].inventory);
            this.searchfocus();
            if (this.searTF == true && ListTab != '' && this.$route.query.Status == 0 || this.searTF == true && ListTab != '' && this.$route.query.Inventory == 0) {
              if (this.listQuery.searchWord == this.list[0].base.barcode && this.listQuery.searchWord != '') {
                let lists = this.list[0]
                this.Stock_change(lists)
                this.loading = false;
              }
              if (this.listQuery.searchWord == this.list[0].base.name && this.listQuery.searchWord != '') {
                let lists = this.list[0]
                this.Stock_change(lists)
                this.loading = false;
              }
      ...
      /*库存修正*/
      submitForm(row, baseId, productId, systemInventory, stocktaking) {
        if (stocktaking === null || stocktaking === '' || stocktaking === undefined) {
        ...
        row.quantity = systemInventory-stocktaking;
        let parment = {
          checkInfo: {checkNo: this.$route.query.coupheckNo},
