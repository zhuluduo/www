  ...
  <div>
    ...
    <el-row class="data-table" :gutter="2">
      <el-col :span="24" class="f-tac check-tit">
        ...
        <div class="f-ib check-tit-pd">盘点状态：<span v-if="this.$route.query.Status==0">正在进行中</span><span
          v-if="this.$route.query.Status==1">已 完 成</span><span v-if="this.$route.query.Inventory==0">正在进行中</span></div>
      </el-col>
    </el-row>
    <el-row class="data-table data-table-pd" :gutter="2">
      ...
      <el-col :span="4">
        <el-input @keyup.enter.native="handleFilter" placeholder="输入商品条码/商品名称" v-model="listQuery.searchWord"
                  size="small"/>
      </el-col>
      <el-col :span="4">
      ...
      <el-col :span="10" align="right">
        ...
        <el-button type="primary" @click="export_check" size="small" icon="caret-right" :loading="loading">导出盘点</el-button>
        <el-button type="primary" @click="End_check" size="small" icon="check" :loading="loading">结束盘点</el-button>
      </el-col>
    </el-row>
    <el-row :gutter="2">
      <el-col :span="24" v-loading="loading">
        <el-table stripe border :data="list">
          ...
          <el-table-column prop="inventory" label="系统库存" sortable></el-table-column>
          <el-table-column prop="stocktaking" label="盘点库存" sortable></el-table-column>
          <el-table-column prop="quantity" label="缺失数量" sortable></el-table-column>
          ...
          <el-table-column label="操作" width="100">
            <template scope="scope">
              <el-button :plain="true" type="warning" @click="Stock_change(scope.row,scope.$index)" size="small" icon="edit">库存修正</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleFilter(){
        this.listQuery.searchWord = $.trim(this.listQuery.searchWord);
        this.listTable();
      },
      /*详情*/
      Stock_change(row){
