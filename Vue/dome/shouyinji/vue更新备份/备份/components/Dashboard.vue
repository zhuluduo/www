    ...
    <el-row class="summary" v-loading="loading">
      ...
      <el-col :span="6" class="summary-center">
        <div class="grid-content">
          ...
          <span class="tag-content" style="padding-left:10px;">{{orderSummary.quantity}}</span>
      ...
      loadData(){
        ...
        this.$axios.post(bus.host+'/pos/api/statistics/order',params).then((res)=>{
          ...
          this.paymethodQuantityChartPie.setOption({
            legend: {
              data: ['现金支付: '+msg.orderSummary.cashQuantity, '微信支付: '+msg.orderSummary.wxQuantity, '支付宝支付: '+msg.orderSummary.alipayQuantity]
            },
            series: [{
              data:[
                { value: msg.orderSummary.cashQuantity, name: '现金支付: '+msg.orderSummary.cashQuantity },
                { value: msg.orderSummary.wxQuantity, name: '微信支付: '+msg.orderSummary.wxQuantity },
                { value: msg.orderSummary.alipayQuantity, name: '支付宝支付: '+msg.orderSummary.alipayQuantity }
              ]}
            ]
          });
