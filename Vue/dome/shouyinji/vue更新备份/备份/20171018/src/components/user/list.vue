  ...
  <div class="content-list">
      ...
      <el-col>
        <el-table stripe border :data="list" v-loading="loading">
          ...
          <el-table-column label="操作" width="230" align="right">
            <template scope="scope">
              <el-button v-if="currentUser.isAdmin==1||scope.row.id==currentUser.id" :plain="true" type="warning" size="small" @click="changePwd(scope.row,scope.$index)" icon="edit">改密</el-button>
              <el-button v-if="currentUser.isAdmin==1&&scope.row.id!=currentUser.id" :plain="true" type="warning" size="small" @click="changePerm(scope.row,scope.$index)" icon="edit">权限</el-button>
              <el-button v-if="currentUser.isAdmin==1&&scope.row.id!=currentUser.id" :plain="true" type="danger" size="small" @click="deleteUser(scope.row,scope.$index)" icon="delete">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
      ...
      <div v-show="showEdit" class="transition-box scrollbar">
        ...
        <el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="ruleForm">
          <el-form-item label="用户名:" prop="username">
            <el-input v-model="ruleForm.username" size="small" :disabled="ruleForm.syncId==''?false:true" />
          </el-form-item>
          <el-form-item label="密码:">
            <el-input type="password" v-model="ruleForm.password" size="small" :rules="[
    ...
    <transition name="bounce">
      <div v-show="showEditPerm" class="transition-box scrollbar">
        ...
        <el-form :model="permForm" ref="permForm" label-width="100px" class="ruleForm">
          <el-form-item>
            <el-checkbox-group v-model="permForm.resources">
        ...
        <el-row class="box-footer">
          <el-col :span="24" align="center">
            <el-button :loading="loading" type="success" @click="submitPermForm('permForm')" size="small" icon="check">保存</el-button>
