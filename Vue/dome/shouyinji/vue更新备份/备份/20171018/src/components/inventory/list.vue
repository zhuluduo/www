    ...
    <el-row>
      <el-col>
        <el-table stripe border :data="list" v-loading="loading"  element-loading-text="数据加载中" @sort-change="sortList" :default-sort = "{prop: 'createTime', order: 'descending'}" show-summary :summary-method="listSummary">
          ...
          <el-table-column sortable="custom" prop="status" label="状态" width="80">
          ...
          <el-table-column label="操作" width="160" align="left">
            <template scope="scope">
              ...
              <el-button v-if="scope.row.status===1" :plain="true" type="danger" size="small" icon="delete" @click="deleteStockIns">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
