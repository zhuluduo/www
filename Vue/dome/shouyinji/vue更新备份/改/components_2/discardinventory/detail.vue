    ...
    <el-row class="breadcrumb-border">
      <el-col>
        <el-breadcrumb separator=">">
          ...
          <el-breadcrumb-item>报损单详情</el-breadcrumb-item>
        </el-breadcrumb>
      </el-col>
    </el-row>
    <el-row class="data-table" :gutter="2">
      <el-col :span="4">报损时间：{{ this.parment.createTime }}</el-col>
      <!--<el-col :span="3">报损人：{{this.$route.params.operator}}</el-col>-->
    ...
    <el-row :gutter="2">
      <el-col :span="24"  v-loading="loading">
        <el-table :data="list" show-summary :summary-method="listSummary">
          ...
          <el-table-column  prop="quantity"  label="报损数量" width="120">
            <template scope="scope">
              <span class="inventory_color">{{scope.row.quantity}}</span>
            </template>
          </el-table-column>
          <el-table-column  prop="reason" label="报损原因" width="300"></el-table-column>
        </el-table>
      </el-col>
    </el-row>
  </div>
    ...
    methods: {
      loadDetail(){
        if(this.$route.query.couponid){
           let url=bus.host + '/pos/api/inventory/scrap/sel?id='
