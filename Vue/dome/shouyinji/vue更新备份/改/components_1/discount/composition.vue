              ...
              <el-col class="Grouptab" :span="24">
                <div v-for="(group,groupIndex) in productGroups" @click="setupGroupIndex(groupIndex)">
                  ...
                  <el-col :span="4" class="fr tool-bar" >
                    ...
                    <!--</el-tooltip>-->
                    <my-component v-on:trans-list="trans" :child-com="groupIndex" :product-groups="productGroups" class="ds-inblok"></my-component>
...
<script>
  ...
  Vue.component('my-component', {
    props: ['childCom','productGroups'],
    template: '  <div> <el-tooltip class="item" effect="dark" content="商品添加" placement="top">\n' +
    '        <el-button type="success" icon="plus" @click="showProductDialog(childCom)" size="small"></el-button>\n' +
    '        </el-tooltip>\n' +
    '           <el-col  :sm="24" :md="24" :lg="24"><el-dialog title="商品添加" :visible.sync="dialogFormVisible" ref=\'dialog\'  @close="show()" >\n' +
    ...
    '           <el-table :data="storage" :show-header="false" style="width: 100%">\n' +
    '           <el-table-column label="" show-overflow-tooltip>\n' +
    '             <template scope="scope" ><el-tag type="primary" >{{scope.row.name}}</el-tag></template>' +
    '             </el-table-column>' +
    '             <el-table-column label=""  width="23">' +
    '             <template scope="scope" >' +
    '             <el-button :plain="true" type="danger" @click="storage.splice(scope.$index, 1)" size="small" icon="delete"></el-button></template>' +
    '             </el-table-column> ' +
    '             </el-table>\n' +
    '           </el-row>\n' +
    '           </el-col>\n' +
    ...
    '                        @selection-change="handleSelectionChange"\n' +
    '                        tooltip-effect="dark" class="tableData_Load">\n' +
    '                <el-table-column type="selection" :reserve-selection="true" :selectable="selectable" width="35"></el-table-column>\n' +
    '                <el-table-column prop="name" label="商品名称"></el-table-column>\n' +
    '                <el-table-column prop="barcode" label="商品条码"></el-table-column>\n' +
    '                <el-table-column prop="spec" label="规格"></el-table-column>\n' +
    '                <el-table-column prop="pkg" label="单位"></el-table-column>\n' +
    ...
    '            <div slot="footer" class="dialog-footer">\n' +
    '              <el-button @click="dialogFormVisible = false" class="fr">取 消</el-button>\n' +
    '              <el-button type="primary" @click="handleClose(childCom)"  class="fr mg_r" >确 定</el-button>\n' +
    '            </div>\n' +
    '          </el-dialog></el-col> </div>',
    data: function () {
      return {
    ...
    methods: {
      ...
      showProductDialog(index){
        ...
        this.getTable();
        this.Primaryone();
//        let pr= this.productGroups[index].items;
//        pr.forEach(row => {
//          console.log(row.id)
//        });
//        this.$refs.multipleTable.toggleRowSelection(this.tableData_Load[2],true);
      },
      /*分页*/
...
//          this.$refs.multipleTable.toggleRowSelection(this.tableData_Load[{2,3}],true);
      ...
      handleClose: function(childCom) {
        this.$emit('trans-list',this.storage , childCom);
